...
def deposit(initial_sum, percent, fixed_period, set_period):
    ...
    percent_1_year = (1 + per) * initial_sum
    ...
    return (f"Yields for 1 yeasr will be: {initial_sum} * {growth_1_year}")
    return (f"Persents for this deposits will be equal to {percent_1_year}")
    return (f"Yields for 5 years will be: {initial_sum} * {growth_5_years}")
    return (f"Yields for 10 yeasr will be: {initial_sum} * {growth_10_yaears}")